...
int expandVector(char ***vector, int *queue_beg, int *queue_end, size_t *size) {
    ...
    for (i = 0; i < *size; i++) {
        ...
        new_vector[i] = malloc(sizeof(char) * (strlen((*vector)[pointer]) + 1));
...
int shift(char ***vector, int *queue_beg, int *queue_end, size_t *size, size_t *actual_size) {
    ...
    // In case the actua size becomes 1/4 of the total, decrease the vector
    if (*actual_size != 0 && *actual_size == (*size) / 4) {
